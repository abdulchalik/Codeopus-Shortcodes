...
function codeopus_run_shortcode( $content ) {
	...
	add_shortcode('cdo_gallery', 'codeopus_gallery_shortcode');
	add_shortcode('cdo_destination', 'codeopus_destination_shortcode' );
...
function codeopus_googlemap($atts, $content = null, $code) {
	...
	$getaddr = ($address!="" ? '<div class="google_map_addr">'.esc_attr($address).'</div>' : '');
		...
		<div style="line-height:0"><script type="text/javascript" src="https://maps.google.com/maps/api/js?sensor=false&key='.esc_attr($apikey).'"></script>
...
function codeopus_products_slider( $atts, $content = null ) {
  ...
  extract(shortcode_atts(array(
	...
	if($layout=='layout2'){
		...
		wp_enqueue_script('cdo-product-slider-slick');
	}else{
		wp_enqueue_script('cdo-jquery-owl-carousel');
		wp_enqueue_script('cdo-jquery-product-slider');
	}

	return cdo_product_slider_function($id, $layout, $title, $subtitle, $subtitle_link, $type, $number, $class);
}
add_shortcode('cdo_products_slider', 'codeopus_products_slider');
}
...
function codeopus_portfolio_shortcode( $atts, $content = null ) {
  extract(shortcode_atts(array(
	...
	wp_enqueue_script( 'cdo-imagesloaded');
	...
	return cdo_portfolio_function($filter, $imagesize, $category, $column, $showpost, $zoomicon, $linkicon, $showtitle, $showdesc, $orderby, $order);
...
function codeopus_pricing_shortcode( $atts, $content = null  ) {
  ...
  extract( shortcode_atts( array(
	...
	$out .='<div class="cdo-pricing-title">';	
	$out .= '<h4 class="p-title">'.$title.'</h4>';
	$out .= '<span class="subtitle">'.$subtitle.'</span>';;
	$out .= '</div>';	
	}
...
function codeopus_testimonial_slider_shortcode( $atts, $content = null  ) {
	...
	return cdo_testimonial_slider_function($layout, $skin, $category, $showpost, $orderby, $order, $color_style, $container_height, $class);

}
add_shortcode( 'cdo_testimonial_slider', 'codeopus_testimonial_slider_shortcode' );
}

if(!function_exists('codeopus_gallery_shortcode')) { 
function codeopus_gallery_shortcode( $atts, $content = null  ) {
	...
	$getcolumn = ($columns!="" ? 'gal'.$columns.'column' : 'gal3column');
	
	$out='';
	if ( $count >= 1 ){
	
		$out.='<ul class="cdo-gallery-container '. esc_attr($getcolumn) .'">';
		foreach ( $attachments as $attachment_id ) {
			$img_src = wp_get_attachment_url($attachment_id);
			$attachment_page = get_attachment_link( $attachment_id ); 
			$img_meta = get_post($attachment_id);
			$img_description = $img_meta->post_title;
			$img_description = ($img_description!="" ? $img_description : '');
			$out.='<li>';
				$out.='<div class="span-img">';
				  $out.='<img src="'.esc_url($img_src).'" alt="'.esc_attr($img_description).'" class="img-full img-100"/>';
				  $out.='<div class="wrap-text-high">';
					$out.='<div class="top-section-high"><a href="'.esc_url($attachment_page).'">'.esc_attr($img_description).'</a></div>';
					$out.='<div class="bottom-section-high">';
					  $out.='<a href="'.esc_url($attachment_page).'"><i aria-hidden="true" class="fa fa-long-arrow-right md-size"></i></a>';
					$out.='</div>';
				  $out.='</div>';
				$out.='</div>';
			$out.='</li>';
...
if (!function_exists('codeopus_destination_shortcode')) { 
function codeopus_destination_shortcode( $atts, $content = null  ) {
	...
	return cdo_destination_function($category, $column, $showpost, $showtitle, $orderby, $order);
